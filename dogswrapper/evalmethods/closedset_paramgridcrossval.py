...
class ParamGridCrossValBase(object):
    ...
    def calculate_p_r_f1(self, crossval_Y, predicted_Y):
        ...
        cv_tg_idxs = np.array(
            np.histogram(crossval_Y, bins=np.arange(self.gnrs_num+2))[0], dtype=np.float
        )
        tp_n_fp = np.array(
            np.histogram(predicted_Y.astype(np.int), bins=np.arange(self.gnrs_num+2))[0],
            dtype=np.float
        )
        ...
        for gnr_cnt in range(len(self.genres_lst)):
            ...
            counts_per_grn_cv = np.histogram(
                predicted_Y[start:end], bins=np.arange(self.gnrs_num+2)
            )[0]
        ...
        if os.path.exists(corpus_files_lst_path) and os.path.exists(corpus_files_lst_path):
            ...
            if gnr_file_idx and iidx:
                ...
                with open(self.corpus_path+gnr_file_idx, 'r') as f:
                    # Getting the list of tuples (index, file) using the index-of-index list...
                    # ...(iidx) argument
                    # iidx[0] == the splittig character, iidx[1] == the field that contains the...
                    # ...filename, iidx[2] == the field that containd the genre of the file
                    gnrs_file_lst = [
                        (line.split(iidx[0])[iidx[2]], line.split(iidx[0])[iidx[1]]) for line in f
                    ]
                ...
                # Build the class-genre-tag list by assigning as a tag the index number of the...
                # ... list of genre given as argument to this class, i.e. ParamGridCrossValBase()
                cls_gnr_tgs = [genres_lst.index(element[0]) for element[1] in gnrs_file_lst]

            elif gnr_file_idx is None and iidx is None:
                ...
                raise Exception(
                    "Both Genre-of-Files-Index and Index-of-Index arguments " +
                    "should be given or 'None' of them"
                )
        ...
        if len(test_only_tgs) > 1:
            ...
            if valid_tgs_lst != test_only_tgs:
                raise Exception(
                    ...
                    "step +1 increment is valid"
                )

            inv_cls_gnr_tgs = cls_gnr_tgs[::-1]
            if inv_cls_gnr_tgs[0:len(test_only_tgs)] != test_only_tgs[::-1]:
                raise Exception(
                    "Invalid test-only-tags sequence: Only the most last tags of the" +
                    "cls_gnr_tgs can be used as only-for-test"
                )
    ...
    def evaluate(self, *args):
        ...
        for k, (trn, crv) in enumerate(KF):
            ...
            not os.path.exists(voc_filename) or not os.path.exists(pkl_voc_filename):
                ...
                tf_d = self.TF_TT.build_vocabulary(
...
class ParamGridCrossValTables(ParamGridCrossValBase):
    ...
    def __init__(self, ML_Model, TF_TT, h5_res, genres, corpus_path, voc_path):
        ...
        super(ParamGridCrossValTables, self).__init__(
            ML_Model, TF_TT, h5_res, genres, corpus_path, voc_path
        )
    ...
    def corpus_matrix(self, k, vocab_size_group, vocab_size, html_file_l, tid, norm_func):
        ...
        if os.path.exists(corpus_mtrx_fname):
            ...
            corpus_mtrx, h5f = self.TF_TT.from_files(
