...
def BuildCombinations(param_vals_ll):
    """ Building all possible combinations given some lists of values in a list. Given several
        lists of values in a python list, this function will return all possible combination of
        the values of these lists. The algorithm producing the  combinations is a recursion over
        the input lists. The output is a list of lists of the combination.
    ...
    if len(param_vals_ll) == 0:
        ...
        for i, param_vals_l in enumerate(param_vals_ll):
            ...
            for param_val in param_vals_l:
                ...
                # Extending the list of the current parameter values with the subsequent...
...
class ParamGridIter(object):
    """ Iterator returning the ordered-dictionaries of parameter value combinations derived given
        ...
        Given a dictionary(or ordered-dictionary) where keys are the parameter names and values are
        the lists of parameter possible values this class defines an iterator which outputs a
        combination of values. The output is a dictionary with parameter names as keys and a values,
        of the possible ones, as a value of the dictionary. The ending condition is the end of the
        list of possible combinations.

        Initialization/Arguments
        ------------------------
            parmas_vals_d: The dictionary of parameters contains the lists of all possible values
            for each parameter.
                ...
                Valid options are:
                    ...
                    'path' : for URL like path (ordered as input dictionary). Compatible to
                             the pyTables path tree.

        Iterations Output
        -----------------
            params_comb_ordered: A possible combinations of parameter values. The output format
                depends on 'mod' value.
    ...
    def __init__(self, params_vals_d, mod='dict'):
        ...
        # Getting the list of parameter names.
        ...
        if mod in ['dict', 'list', 'array', 'path']:
            ...
            raise Exception(
                "Invalid value for 'mod' initialization argument: Valid values are " +
                "'dict', 'lst', 'array', 'path'."
            )

        # Initializing the counter of all possible combination.
        self.combs_cnt = -1
    ...
    def next(self):
        ...
        if self.mod == 'dict':
            ...
            # Convert the combination values list into combination ordered dictionary.
            params_comb_ordered = coll.OrderedDict(
                zip(self.params_lst, self.params_vals_combs[self.combs_cnt])
            )
            ...
            params_path = '/'.join(
                [
                    key+str(value).replace('.', '') for key, value in
                    zip(self.params_lst, self.params_vals_combs[self.combs_cnt])
                ]
            )
